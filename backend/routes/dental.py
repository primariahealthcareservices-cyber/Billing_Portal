...
def _clean(value):
    """Trim strings; return None for empty/None."""
    if value is None:
        return None
...
def _resolve_category(data):
    """
    Returns the actual category to store.

    The form sends:
      - category: "Others" + other_category: "Custom Name"  -> store "Custom Name"
      - category: "Normal Category"                         -> store it directly
    """
...
def _apply_filters(query):
    """Apply date / type / category / revenue_type / search filters."""
    args = request.args
...
# ---------------------------------------------------------------------------
# Summary (KPIs + trend + category breakdown)
# ---------------------------------------------------------------------------
...
def create_entry():
    ...
    # Validate category belongs to the chosen entry_type (unless custom).
