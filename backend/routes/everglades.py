...
def _parse_items_field(raw_items):
    """Handle items that arrive either as a JSON string (FormData) or a list (JSON body)."""
...
def update_entry(entry_id):
    ...
    # ✅ THE FIX: accept BOTH JSON and multipart/form-data.
    if request.is_json:
        data = request.get_json(silent=True) or {}
    else:
        data = request.form  # ImmutableMultiDict works with .get() and "in"
    ...
    if new_category == "Goodwill":
        ...
        if require_items:
            ...
            if items_changed:
                # ✅ Handle items that arrive as a JSON string (FormData) or list (JSON)
                items_data = _parse_items_field(data.get("items"))
                if items_data is None:
                    items_data = []
                    errors.append("Items data could not be read.")
